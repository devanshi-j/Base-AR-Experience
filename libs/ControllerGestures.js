...
class ControllerGestures extends THREE.EventDispatcher {
    constructor(renderer) {
        ...
        this.controller1.addEventListener('selectstart', onSelectStart.bind(this));
        this.controller1.addEventListener('selectend', onSelectEnd.bind(this));

        this.controller2 = renderer.xr.getController(1);
        this.controller2.userData.gestures = { index: 1 };
        this.controller2.userData.selectPressed = false;
        this.controller2.addEventListener('selectstart', onSelectStart.bind(this));
        this.controller2.addEventListener('selectend', onSelectEnd.bind(this));
    ...
    get multiTouch() {
        ...
        console.log(`ControllerGestures multiTouch: ${result} touchCount:${this.touchCount}`);
        return result;
    }
    ...
    update() {
        ...
        if (this.type === 'unknown' && this.touch) {
            if (data1.startPosition !== undefined) {
                if (this.multiTouch) {
                    ...
                    if (this.type === 'rotate') {
                        ...
                        if (this.type === 'rotate') {
                            this.handleRotation(this.up, theta); // Rotate around the up axis
            ...
            this.dispatchEvent({ type: 'rotate', theta });
            if (this.type === 'rotate') {
                this.handleRotation(this.startVector, v);
                this.startVector = v.clone(); // Update startVector for subsequent rotations
            }
